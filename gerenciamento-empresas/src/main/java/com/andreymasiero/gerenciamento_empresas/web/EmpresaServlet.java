...
public class EmpresaServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) 
		...
		String nomeEmpresa = req.getParameter("novaEmpresa");
		Empresa empresa = new Empresa(nomeEmpresa);
			new EmpresaDAO().adiciona(empresa);
			req.setAttribute("sucesso", "Adicionado com sucesso");
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) 
		...
		String busca = req.getParameter("buscaEmpresa");
		Collection<Empresa> similares = new EmpresaDAO().buscaPorSimilaridade(busca);
		if(similares.size() == 1) {
			req.setAttribute("busca", busca);
			req.getRequestDispatcher("gerenciador.jsp").forward(req, resp);
		}else {
			req.setAttribute("similares", similares);
			//req.getRequestDispatcher("gerenciador.jsp").forward(req, resp);
			req.getSession().setAttribute("similares", similares);
